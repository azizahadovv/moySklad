...
public class WebSettingsController {
    ...
    public List<Map<String, Object>> menus(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init) {
        superadmin(init); return svc.menus();
    }

    @PutMapping("/settings/menu/{key}")
    public List<Map<String, Object>> saveMenu(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init,
    ...
    public List<Map<String, Object>> saveSchema(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init, @RequestBody SchemaReq r) {
        svc.saveSchema(superadmin(init), r.menus()); return svc.menus();
    }

    @PostMapping("/settings/menu/{key}/reset")
    public List<Map<String, Object>> resetMenu(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init,
    ...
    public List<Map<String, Object>> labels(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init) {
        superadmin(init); return svc.labels();
    }

    @PutMapping("/settings/labels")
    public List<Map<String, Object>> saveLabel(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init,
    ...
    public List<Map<String, Object>> setRolePerm(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init, @RequestBody RolePermReq r) {
        svc.setRolePerm(superadmin(init), r.role(), r.canonical(), r.state()); return svc.menus();
    }

    @GetMapping("/settings/perm")
    public Map<String, Object> perms(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init) {
        superadmin(init); return svc.perms();
    }

    @GetMapping("/settings/perm/{subj}/{id}")
    public Map<String, Object> permOf(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init,
                                      @PathVariable String subj, @PathVariable long id) {
        superadmin(init); return svc.permOf(subj, id);
    }

    @PutMapping("/settings/perm")
    public Map<String, Object> setPerm(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init, @RequestBody PermReq r) {
    ...
    @GetMapping("/notify")
    public List<Map<String, Object>> notifies(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init) {
        superadmin(init); return svc.notifies();
    }

    @GetMapping("/notify/help")
    public Map<String, Object> help(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init) {
        superadmin(init); return svc.help();
    }

    @PutMapping("/notify/confirm-delete")
    public Map<String, Object> confirmDelete(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init, @RequestBody MinReq r) {
        svc.setConfirmDeleteMin(superadmin(init), r.min()); return svc.help();
    }

    @GetMapping("/notify/presets")
    public List<Map<String, Object>> presets(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init) {
        superadmin(init); return svc.presets();
    }

    @PostMapping("/notify/presets/{key}")
    public Map<String, Object> fromPreset(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init, @PathVariable String key) {
        return svc.createFromPreset(superadmin(init), key);
    }

    @PostMapping("/notify/preview")
    public Map<String, Object> preview(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init, @RequestBody PreviewReq r) {
        return svc.preview(superadmin(init), r.template());
    }

    @GetMapping("/notify/{id}")
    public Map<String, Object> notify(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init, @PathVariable long id) {
        superadmin(init); return svc.notify(id);
    }

    @PostMapping("/notify")
    public Map<String, Object> create(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init,
                                      @RequestBody AdminSettingsService.NotifyReq r) {
        return svc.saveNotify(superadmin(init), null, r);
    }

    @PutMapping("/notify/{id}")
    public Map<String, Object> update(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init, @PathVariable long id,
                                      @RequestBody AdminSettingsService.NotifyReq r) {
        return svc.saveNotify(superadmin(init), id, r);
    }

    @DeleteMapping("/notify/{id}")
    public Map<String, Object> delete(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init, @PathVariable long id) {
        svc.deleteNotify(superadmin(init), id); return Map.of("ok", true);
    }

    @PostMapping("/notify/{id}/send")
    public Map<String, Object> send(@RequestHeader(name = "X-Telegram-Init-Data", required = false) String init, @PathVariable long id,
